...
In the following table, *GA* and *Paid preview* mean that the feature is available and uses an Azure OpenAI service in the same region. Learn more in [enabling data movement cross-regions](/power-platform/admin/geographical-availability-copilot#enable-data-movement-across-regions).
