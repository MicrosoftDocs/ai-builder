---
title: AI Builder licensing and AIB credit management
description: Learn about licensing and AIB credit management in AI Builder.
author: Antoine2F
contributors:
  - Antoine2F
  - chplanty
...
# AI Builder licensing and AIB credit management
...
Once an environment has access to AIB credits, AI Builder features are enabled in this environment.

Some AI Builder actions consume AIB credits. Learn more in [AI Builder credit consumption](#ai-builder-credit-consumption) in this article. AIB Credit consumption can be monitored. Learn more in [Monitor usage](#monitor-usage) in this article.

In an environment, when consumption exceeds available credits, the environment is in overage and some features are progressively blocked. You need to purchase AIB credits and/or reassign AIB credits. Learn more in [Overage](#overage) in this article.
...
| Power Apps Premium |500| Maximum = 1,000,000 AIB credits per tenant.|
| Power Apps per app |250| Maximum = 1,000,000 AIB credits per tenant. Per app licenses purchased before November 2022 don't include any credits.|
| Power Automate Premium |5,000| Maximum = 1,000,000 AIB credits per tenant.|
| Power Automate Process |5,000| Maximum = 1,000,000 AIB credits per tenant.|
| Power Automate Hosted RPA add-on |5,000| Maximum = 1,000,000 AIB credits per tenant.|
| Power Automate Unattended RPA add-on |5,000| Maximum = 1,000,000 AIB credits per tenant.|
| Dynamics 365 F&O|20,000| Maximum = 20,000 AIB credits per tenant. |
| Power Apps for Cloud for Sustainability USL Plus | 25,000 AIB credits| None.|

### Entitlement through AI Builder trial

Entitlement can be through a trial. AIB Credits are then linked to a user. Trials are available only when there's no paid capacity within the organization (tenant).

In a trial, there's no need for allocating AIB credits to an environment. Trials can be used in any environment. A trial brings 200,000 AIB credits and is valid 30 days.

Once AIB credits are consumed, AI Builder models are blocked for this user.
At the end of the 30 days, a trial can be renewed, and user receives another 200,000 AIB credits. Renewal is only allowed for a limited number of times.

Learn more in [AI Builder trial](ai-builder-trials.md).

## Make AIB credits available for an environment: allocated and unallocated credits

By default, the AIB credits are unallocated (also known as unassigned) and available as a pool on the organization (tenant), which can be used on any environment.
...
### Block use of unallocated AIB credits

The administrator can block the use of unallocated AIB credits through an organization (tenant) setting in [Power Platform admin center](https://admin.powerplatform.microsoft.com/). To do this, remove the check from the **Allow users to consume unassigned AIB credits** checkbox.

:::image type="content" source="media/credit-management/credits.png" alt-text="Screenshot of the option to allow users to consume unassigned AIB credits in Power Platform admin center.":::

By default, this setting is enabled. When disabled, only environments with allocated AIB credits have access to AI Builder features.
...
Learn more about how to allocate AIB credits in the Power Platform admin center in [Allocate or change capacity in an environment](/power-platform/admin/capacity-add-on#allocate-or-change-capacity-in-an-environment).

## Monitor usage and understand AIB credit consumption

You can gain insights into your AI Builder credit consumption by monitoring usage.

### Monitor usage

As an administrator, you can view your overall AIB credit consumption in the [Power Platform admin center](https://admin.powerplatform.microsoft.com/) in the **Licensing** > **Capacity add-ons** > **Summary** tab.  

The **Allocation bar** and the **Consumption bar** display the number of **allocated** AIB credits and **consumed** AIB credits out of the total number of **purchased** AIBcredits.
...
AIB Credit extensions aren't counted in *purchased* AIB credits.

Administrators also have access to a [consumption report](administer-consumption-report.md) that has the following features:

- Provides the AIB credits consumption on a chosen period per environment.
- Allows you to fine-tune the AIB credits allocation, which can be updated at any time.
...
The following list presents AI Builder actions that consume AIB credits. It isn't all-inclusive.
...
Each AI Builder model has a different AIB credit consumption mechanism. To perform an assessment, use the AI Builder rate card in [Microsoft Power Platform Licensing Guide](https://go.microsoft.com/fwlink/?linkid=2085130) (pdf).

#### Free actions

Preview scenarios in AI Models don't consume credits. However, using a prompt in Power Automate or Power Apps consumes AIB credits, even if the underlying model is preview: such models are tagged as 'paid preview' in prompt builder.
...
In the context of Power Apps and Power Automate flows, AI prompts consume AIB credits.

The number of consumed AIB credits depends on the input tokens, output tokens, and the underlying generative AI model used. Learn more in [Prompt tokens](licensing-prompt-tokens.md).

## Overage

When browsing AI Builder pages in Power Apps or Power Automate portal, you might get this notification: **You've consumed all of your AI Builder credits. Creating, editing, and viewing is blocked. Contact your administrator to get more capacity**. This notification means the monthly consumption of your environment is higher than the available (at environment or organization -tenant- level) AIB credits.

Monthly consumption is computed from first day of the month and is refreshed on a daily basis.
  
Available AIB credits are the assigned AIB credits if AIB credits are assigned to the environment, or the unassigned AIB credits if no AIB credit is assigned to the environment.  

### Simple overage

As soon as monthly consumption is higher than the available AIB credits, creating and editing models is blocked.
...
As long as overage isn't important, AI Builder models continue to run and consume AIB credits. We allow some level of overage consumption&mdash;similar to a grace period&mdash;to avoid blocking business processes.

### Important overage

When monthly consumption becomes higher than the available AIB credits by a large margin, running models in flows and apps is blocked. In this state, AI Builder actions fail with `EntitlementNotAvailable` `QuotaExceeded` error codes. In flow editor, remediation panel displays **All AI Builder credits in this environment have been consumed**.
...
### Request AIB credit extension

In the case of simple or important overage, and when another purchase and/or reallocation of AIB credit can't be done in a timely manner, makers in AI Builder and admins in the [Power Platform admin center](https://admin.powerplatform.microsoft.com/home) can request a free AIB credit extension.
...
- Add 200,000 free extension AIB credits for the current month.

These free temporary AIB credits can't be allocated or assigned. Any environment in overage can use them after 10 minutes.

The number of purchased AIB credits in the Allocation bar or in the Consumption bar isn't modified with these free temporary AIB credits. Learn more in [Monitor usage](credit-management.md#monitor-usage). These added temporary AIB credits don't appear in [Power Platform admin center](https://admin.powerplatform.microsoft.com/home).

In some cases, you might still see the overage banners, although extension AIB credits already unblock your models.

Depending on the level of overage, these 200,000 AIB credits might not be enough. They can be requested multiple times, but the number of extension requests per month and per year is limited. Each request grants 200,000 AIB credits and triggers an email.

The purpose of these free AIB credit extensions is to allow makers and admin to unblock themselves temporarily in case another purchase and/or reallocation can't be done in a timely manner. Customer shouldn't rely on credit extensions to cover repetitive overage. Repetitive overage must be managed with another purchase and/or reallocation.
...
Each AI Builder capability consumes AIB credits at a different rate. You can check the consumption rates in the [Microsoft Power Platform Licensing Guide](https://go.microsoft.com/fwlink/?LinkId=2085130) in the **AI Builder capacity add-on** section.
...
### I have seeded AIB credits. How many add-ons should I buy to cover my need?

AIB Credits coming from the AI Builder add-on (1,000,000 credits per add-on) and AIB credits coming from licenses with seeded capacity (like Power Automate premium, which brings 5,000 credits) are gathered at the tenant level. They represent your entitlement.

Example 1:

One (1) add-on + 5 Power Automate premium licenses gives 1,000,000+5*5,000 = 1,025,000 AIB credits that you can assign and consume in any scenario.

Example 2:

You have a scenario with 32,000 receipts to process each month with receipt processing.

32,000 receipts require 32 AIB credits per receipt (exact rates are in the [Microsoft Power Platform Licensing Guide](https://go.microsoft.com/fwlink/?linkid=2085130)), so you have 1,024,000 AIB credits.

Suppose you own five (5) Power Automate premium licenses, you're covered with a single add-on:

One (1) add-on + 5 Power Automate premium licenses = 1,025,000 AIB credits, which is greater than your estimated consumption of 1,024,000 AIB credits.

### Where can I see AIB credit usage for this month?

Overall organization (tenant) level AIB credit consumption is available in the [Power Platform admin center](https://admin.powerplatform.microsoft.com/) in the **Resources** > **Capacity** > **Summary** tab. The Consumption bar displays the number of *consumed* AIB credits out of the total of *purchased* AIB credits.
...
### How do I find out which model is using the most AIB credits?

Once you identify which environment consumes the most AIB credits with the [AI Builder consumption report](administer-consumption-report.md), you can leverage the model consumption data in the Dataverse **AI Event** table of that environment either in the Power Apps portal or the [AI Builder Activity](activity-monitoring.md) page in Power Automate.

### How do I know the number of purchased AIB credits of my organization (tenant)?

The [consumption bar and allocation bar](https://go.microsoft.com/fwlink/?linkid=2255314)  display the number of *purchased* AIB credits.

AIB Credits come from AI Builder add-ons and are also seeded in premium licenses like Power Automate Premium.
...
### When is AIB credit usage count reset? Is it based on license acquisition date?

AIB Credit usage is counted on a monthly basis, starting the first day of each month. It isn't based on the license acquisition date. AIB Credit usage of an environment shouldn't exceed the AIB credit allocation.  

### What happens to the remaining capacity at the end of the month? Is it used the following month?

No, unused capacity doesn't carry over to the next month. AIB Credit usage is counted on a monthly basis and reset on first day of the month. Available capacity is based on allocated capacity.

### What can I do with the 5,000 AIB credits included in the per user plan with attended RPA?

Each user license grants you 5,000 AIB credits, allowing you to assess the capabilities in AI Builder. For instance, you could use these AIB credits to extract data from a few documents with [document processing](form-processing-model-overview.md) or perform hundreds of basic OCR extractions with [text recognition](prebuilt-text-recognition.md).
...
### What are the differences between AIB credit assignment and credit AIB allocation?

AIB Credit assignment and AIB credit allocation are same concept. In documentation, we mainly use allocation. In product, we mainly use assignment.
...
### What are the differences between allocated AIB credits and unallocated AIB credits?

When an organization (tenant) purchases AI Builder capacity, corresponding AIB credits are by default unallocated and available as a pool for the organization (tenant). In this state, AIB credits can be used on any environment without assigned credits, unless admin blocks usage of unallocated AIB credits using the ['AI Builder Credit'](#block-use-of-unallocated-credits) organization (tenant) setting.

The administrator can restrict usage by allocating all AIB credits to specific environments or by blocking the usage of unallocated AIB credits. They can reserve some capacity to an environment by allocating AIB credits to this environment.

Environments with allocated AIB credits only consume allocated AIB credits. There's no automatic switch to unallocated AIB credit consumption when in overage. Environments without allocated AIB credits only consume unallocated AIB credits.
...
### Is there a way to automatically consume organization (tenant) level AIB credits once environment allocated AIB credits are consumed?

No, AIB credits either come from environment allocation or comes from organization (tenant) level unallocated AIB credits but there's no automatic switch from environment credits to organization (tenant) AIB credits.

As soon as environment allocates AIB credits, AIB credits only come from environment AIB credits.
If environment consumption is higher than allocated AIB credits, then environment is in overage.

### How do you consider the 'large margin' to block model runs in important overage?

For an environment with allocated AIB credits, we apply the ‘large margin’ when comparing allocated AIB credits and consumed AIB credits. We don't consider organization (tenant) level AIB credits.
For an environment without allocated AIB credits, we apply the ‘large margin’ when comparing unallocated AIB credits and consumed AIB credits.

- **Simple overage**: Limitation starts as soon as consumed credits are higher than available AIB credits (no margin here) and only impacts maker experience. (creating/editing models)
...
For example, if an environment has no allocated credit, and organization (tenant) level AIB credits aren't all consumed, then editing and running of AI Builder functions are allowed for this environment. This remains true when other environments with assigned AIB credits are in overage.

Consumption of unassigned AIB credits never affectS the consumption of an environment with assigned AIB credits, even if it's in overage. Consumption of credits in an environment with assigned AIB credits never affectS the consumption of AIB credits in other environments, even if it's in overage.
